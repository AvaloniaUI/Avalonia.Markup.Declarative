...
public static class MvuComponentExtensions
{
    /// <summary>
    ...
    public static TControl _setCommonBindingEx<TControl, TValue>(this TControl control, Action<TValue> setAction, IBinding ibinding)
        where TControl : AvaloniaObject
    {
        if (ibinding is MvuComponentBase.MvuBinding bindingEx)
        {
            TValue val = (TValue)bindingEx.Value;
            setAction(val);

            if (control is MvuComponentBase targetView)
            {
                targetView.UpdateState();

                if (bindingEx.Source is MvuComponentBase sourceView)
                {
