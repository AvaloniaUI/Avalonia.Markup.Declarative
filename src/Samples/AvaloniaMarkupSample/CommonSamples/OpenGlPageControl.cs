...
");
    ...
    {
        var name = typeof(OpenGlView).Assembly.GetManifestResourceNames().First(x => x.Contains("teapot.bin"));
        using var sr = new BinaryReader(typeof(OpenGlView).Assembly.GetManifestResourceStream(name)!);
    ...
    protected override unsafe void OnOpenGlRender(GlInterface gl, int fb)
    {
        ...
        float dpiScaling = (float)(VisualRoot?.RenderScaling ?? 1.0f);

        // Calculate adjusted width and height based on DPI scaling
        int width = (int)(Bounds.Width * dpiScaling);
        int height = (int)(Bounds.Height * dpiScaling);

        // Set viewport with adjusted width and height
        ...
        // Adjust the projection matrix for the new viewport dimensions
        var projection = Matrix4x4.CreatePerspectiveFieldOfView((float)(Math.PI / 4), (float)(width / height), 0.01f, 1000);

        // Define view and model matrices
